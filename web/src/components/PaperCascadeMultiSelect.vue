  ...
  return Array.from(bySubject.keys())
    ...
    .map((subject) => {
      ...
      const years = Array.from(yearMap.keys()).sort(sortYears).map((year) => {
        ...
        return {
          key: year,
          label: year === 'unknown' ? t('pcms.unknownYear') : year,
...
const displayLabel = computed(() => {
  ...
  if (selected <= 0 || (total > 0 && selected >= total)) return props.placeholder || t('pcms.placeholder')
  if (selected === 1) {
    const target = normalizedOptions.value.find((x) => selectedSet.value.has(x.value))
    return target?.label || props.placeholder || t('pcms.placeholder')
  }
  return t('pcms.selectedCount', { n: selected })
})
...
function parsePaperMeta(label: string): { subject: string; year: string; season: string } {
  ...
  const subject = (text.match(/^([A-Za-z0-9]+)/)?.[1] || t('pcms.unknownSubject')).trim()
          ...
          <div class="pcms-head">
            ...
            <div class="pcms-actions">
              ...
              <button class="pcms-action-btn" type="button" @mousedown.prevent @click="selectAllFiltered">
                {{ filteredOptions.length && filteredOptions.every((p) => selectedSet.has(p.value)) ? t('pcms.clearCur') : t('pcms.selectAllCur') }}
              </button>
              <button class="pcms-action-btn" type="button" @mousedown.prevent @click="clearAll">{{ t('pcms.clearAll') }}</button>
            </div>
          </div>

          <div v-if="grouped.length" class="pcms-grid">
            <div class="pcms-col pcms-col--subject">
              <div class="pcms-col-title">{{ t('pcms.colSubject') }}</div>
              <button
            ...
            <div class="pcms-col pcms-col--year">
              <div class="pcms-col-title">{{ t('pcms.colYear') }}</div>
              <button
            ...
            <div class="pcms-col pcms-col--season">
              <div class="pcms-col-title">{{ t('pcms.colSeason') }}</div>
              <button
            ...
            <div class="pcms-col pcms-col--paper">
              ...
              <div v-if="!currentPaperList.length" class="pcms-empty">{{ t('pcms.emptyPapers') }}</div>
            </div>
          </div>

          <div v-else class="pcms-empty pcms-empty--panel">{{ t('pcms.noMatch') }}</div>
        </div>
      </Transition>
    </Teleport>
  </div>
